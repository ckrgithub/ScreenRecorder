...
public class PermissionRequest {
    ...
    public static boolean requestReadPermission(@NonNull final Activity activity) {
        ...
        if (!isGrant) {
            if (activity.shouldShowRequestPermissionRationale(READ_CONTACTS)) {//选择了拒绝并且不再提醒  return true
    ...
    public static boolean requestReadPermission(@NonNull final Fragment fragment) {
        ...
        if (!isGrant) {
            if (fragment.shouldShowRequestPermissionRationale(READ_CONTACTS)) {//选择了拒绝并且不再提醒  return true
